...
def bce_loss(output, target, classes=None, device=None, **kwargs):
    ...
    return F.binary_cross_entropy_with_logits(output.squeeze(), target.to(device, dtype=torch.double).squeeze(), **kwargs)
...
def focal_loss(output, target, classes=None, device=None, **kwargs):
    ...
    ce_loss = F.cross_entropy(output.squeeze(), target.squeeze(), reduction='none')
...
def binary_focal_loss(output, target, classes=None, device=None, **kwargs): # only for binary classification
    ...
    return sigmoid_focal_loss(output.squeeze(), target.to(dtype=torch.float, device=device).squeeze(), **kwargs)
...
def soft_margin_loss(output, target, classes=None, device=None, **kwargs):
    ...
    return F.soft_margin_loss(output.squeeze(), target.to(dtype=torch.float, device=device).squeeze(), **kwargs)
...
def hinge_embedding_loss(output, target, classes=None, device=None, **kwargs):
    ...
    return F.hinge_embedding_loss(output.squeeze(), target.to(dtype=torch.float, device=device).squeeze(), **kwargs)
...
def binary_auc_marging_loss(output, target, classes=None, device=None, **kwargs):
    ...
    return loss_fn(output.squeeze(), target.squeeze())
