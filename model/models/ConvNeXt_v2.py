...
from utils import load_timm_model, change_kwargs # for pre-trained model
...
def convnextv2_femto(pretrained:bool=False, **kwargs):
    ...
    if pretrained:
        # https://huggingface.co/timm/convnextv2_femto.fcmae_ft_in1k
        num_classes, in_chans, _ = change_kwargs(**kwargs)
        model = load_timm_model('convnextv2_femto.fcmae_ft_in1k', pretrained=True, num_classes=num_classes)
        if in_chans is not None: model.stem[0] = nn.Conv2d(in_chans, model.stem[0].out_channels, 
                                                           kernel_size=model.stem[0].kernel_size, stride=model.stem[0].stride)
    else: 
...
def convnextv2_pico(pretrained:bool=False, **kwargs):
    ...
    if pretrained:
        # https://huggingface.co/timm/convnextv2_pico.fcmae_ft_in1k
        num_classes, in_chans, _ = change_kwargs(**kwargs)
        model = load_timm_model('convnextv2_pico.fcmae_ft_in1k', pretrained=True, num_classes=num_classes)
        if in_chans is not None: model.stem[0] = nn.Conv2d(in_chans, model.stem[0].out_channels, 
                                                           kernel_size=model.stem[0].kernel_size, stride=model.stem[0].stride)
    else: 
...
def convnextv2_nano(pretrained:bool=False, **kwargs):
    ...
    if pretrained:
        # https://huggingface.co/timm/convnextv2_nano.fcmae_ft_in1k
        num_classes, in_chans, _ = change_kwargs(**kwargs)
        model = load_timm_model('convnextv2_nano.fcmae_ft_in1k', pretrained=True, num_classes=num_classes)
        if in_chans is not None: model.stem[0] = nn.Conv2d(in_chans, model.stem[0].out_channels, 
                                                           kernel_size=model.stem[0].kernel_size, stride=model.stem[0].stride)
    else: 
...
def convnextv2_tiny(pretrained:bool=False, **kwargs):
    ...
    if pretrained:
        # https://huggingface.co/timm/convnextv2_tiny.fcmae_ft_in1k
        num_classes, in_chans, _ = change_kwargs(**kwargs)
        model = load_timm_model('convnextv2_tiny.fcmae_ft_in1k', pretrained=True, num_classes=num_classes)
        if in_chans is not None: model.stem[0] = nn.Conv2d(in_chans, model.stem[0].out_channels, 
                                                           kernel_size=model.stem[0].kernel_size, stride=model.stem[0].stride)
    else: 
...
def convnextv2_base(pretrained:bool=False, **kwargs):
    ...
    if pretrained:
        # https://huggingface.co/timm/convnextv2_base.fcmae_ft_in1k
        num_classes, in_chans, _ = change_kwargs(**kwargs)
        model = load_timm_model('convnextv2_base.fcmae_ft_in1k', pretrained=True, num_classes=num_classes)
        if in_chans is not None: model.stem[0] = nn.Conv2d(in_chans, model.stem[0].out_channels, 
                                                           kernel_size=model.stem[0].kernel_size, stride=model.stem[0].stride)
    else: 
...
def convnextv2_large(pretrained:bool=False, **kwargs):
    ...
    if pretrained:
        # https://huggingface.co/timm/convnextv2_large.fcmae_ft_in1k
        num_classes, in_chans, _ = change_kwargs(**kwargs)
        model = load_timm_model('convnextv2_large.fcmae_ft_in1k', pretrained=True, num_classes=num_classes)
        if in_chans is not None: model.stem[0] = nn.Conv2d(in_chans, model.stem[0].out_channels, 
                                                           kernel_size=model.stem[0].kernel_size, stride=model.stem[0].stride)
    else: 
...
def convnextv2_huge(pretrained:bool=False, **kwargs):
    ...
    if pretrained:
        # https://huggingface.co/timm/convnextv2_huge.fcmae_ft_in1k
        num_classes, in_chans, _ = change_kwargs(**kwargs)
        model = load_timm_model('convnextv2_huge.fcmae_ft_in1k', pretrained=True, num_classes=num_classes)
        if in_chans is not None: model.stem[0] = nn.Conv2d(in_chans, model.stem[0].out_channels, 
                                                           kernel_size=model.stem[0].kernel_size, stride=model.stem[0].stride)
    else: 
