...
class Result_visualization:
    def __init__(self, parent_dir, result_name, best_metric):
        ...
        # metric.json 기반으로 정보 dataframe으로 변환
    ...
    def _get_a_result_info(self):
        ...
        for category, cate_path in cate_model_info.items():
            for model, model_path in cate_path.items():
                ...
                for metrics_cate, metrics_path in metrics_None.items():
                    ...
                    # 테스트 정보를 모두 가져온다.
                    test_json = sorted((metrics_path/'test').glob('metrics*.json'))
                    if len(test_json) >= 1:
                        test_epochs = []
                        for t in test_json:
                            test_epoch = str(t).split('epoch')[-1].split('.json')[0]
        ...
        print(f'example result file : {metrics_dict[use_cate][use_model][use_run]}')
        return metrics_dict
    # output 중, metric.json 경로 정보 셋팅하는 함수
