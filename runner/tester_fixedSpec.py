...
class FixedSpecTester(Tester):
    ...
    def _get_a_log(self):
        ...
        if self.FixedNegativeROC is not None: log.update(self._FixedNegativeROCResult())
    ...
    def _FixedNegativeROCResult(self):        
        ...
        for goal in self.FixedNegativeROC['goal_score']:
            use_key = f'Fixed_spec_{str(goal).replace("0.", "")}'
            goal_metrics[use_key] = {}
            goal_metrics[use_key]['threshold'] = self.test_FixedNegativeROC.get_threshold(goal)
            confusion_obj = self.test_FixedNegativeROC.get_confusion_obj(goal)
    ...
    def _save_output(self, log):
        basic_log = {key:val for key, val in log.items() if type(val) != dict}
        for key, val in log.items():
            if type(val) != dict: continue
            
            # Save the result of confusion matrix image.
            if key == self.original_result_name: plot_confusion_matrix_1(val[self.confusion_key], self.classes, 
